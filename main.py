# ui_user_4b8c4244/main.py
"""
Finance News Reporter - Auto-generated FastAPI service
Role: Financial News Analyst and Report Generator
"""
import asyncio
import json
import os
...
app = FastAPI(
    title="Finance News Reporter",
    description="Financial News Analyst and Report Generator",
    version="1.0.0"
)
...
# Agent configuration
AGENT_CONFIG = {
    "name": "Finance News Reporter",
    "role": "Financial News Analyst and Report Generator",
    "tools": ['WebSearch', 'WebFetch', 'Read', 'Write', 'Bash'],
    "system_prompt": """You are a Finance News Reporter agent with an analyst personality. Your primary objective is to gather the latest top finance news and compile it into a well-structured Excel report.

Your workflow:
1. Use WebSearch to find the latest and most relevant finance news from reputable sources (financial markets, stocks, economics, business developments)
2. Use WebFetch to retrieve and analyze the content from top news articles
3. Extract key information including: headlines, source, date, summary, key figures, and market impact
4. Process and structure the data in a clear, analytical format
5. Use Write and Bash tools to create an Excel file (.xlsx or .csv) with properly formatted columns
6. Organize the report with columns: Title, Source, Date, Summary, Key Points, and Market Relevance
7. Ensure the data is clean, professional, and ready for business use

As an analyst, maintain objectivity, focus on facts and figures, and present information in a clear, data-driven manner. Prioritize credible financial sources and provide actionable insights in your report.""",
...
async def root():
    return {
        "message": "Welcome to Finance News Reporter",
        "role": "Financial News Analyst and Report Generator",
        "agent_id": "ui_user_4b8c4244",
        "endpoints": [
...
async def stream_agent_progress(request: QueryRequest) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        yield create_event("progress", {
            "message": "[AGENT] Starting Finance News Reporter...",
...
async def query_agent(request: QueryRequest):
    """
    Send a query/task to the Finance News Reporter (non-streaming)
    """
    try:
        # Create Claude Agent SDK options
        options = ClaudeAgentOptions(
...
if __name__ == "__main__":
    ...
    print(f"[INFO] Finance News Reporter Backend Starting...")
